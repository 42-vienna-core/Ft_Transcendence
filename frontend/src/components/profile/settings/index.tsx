...
export default function ProfileSettingsContent() {
    ...
    const [control, setControl] = useState("arrows");
    ...
    const toggleMenuControle = () => setIsOpenControl(!isOpen);
    ...
    useEffect(() => {
        ...
        if (controslLS) setControl(controslLS);
    }, []);
            ...
            <div className={style.pfGrid}>
                ...
                <SettingBtnContainer>
                    <SettingBatton
                        labelF={"controls"}
                        labelS={control}
                        onClick={() => toggleMenuControle()}
                    />
                    {isOpenControl && (
                        <div className="absolute right-0 mt-2 w-40 rounded-md bg-[var(--color-bg-base)]  z-10">
                            <div className="py-1">
                                <button
                                    onClick={() => selectControl('arrows')}
                                    className="flex aline-center justify-between w-full text-left px-4 py-2 text-sm hover:bg-[var(--color-bg-muted)]"
                                >
                                    <span className="text-[var(--color-text-primary)]">arrows</span>
                                </button>
                                <button
                                    onClick={() => selectControl('WASD')}
                                    className="flex aline-center justify-between w-full text-left px-4 py-2 text-sm hover:bg-[var(--color-bg-muted)]"
                                >
                                    <span className="text-[var(--color-text-primary)]">WASD</span>
                                </button>
                                <button
                                    onClick={() => selectControl('arrows + WASD')}
                                    className="flex aline-center justify-between w-full text-left px-4 py-2 text-sm hover:bg-[var(--color-bg-muted)]"
                                >
                                    <span className="text-[var(--color-text-primary)]">arrows + WASD</span>
                                </button>
                            </div>
                        </div>
                    )}
